...
class PreviewView extends View {
  ...
  _generateMarkup() {
    if (this._data.destinyMemberships.length) {
      ...
      return `
      <li class="profile-search-result">
        ...
        <img
          ...
          alt="Platform Logo"
          class="profile-search-result-platform-img"
        />
        </a>
      </li>`;
    } else {
      return `
      <li class="profile-search-result-no-platform">
        <a class="profile-name-no-platform" href="#">${
          this._data.bungieGlobalDisplayName
        }#${this._data.bungieGlobalDisplayNameCode.toString().padStart(4, '0')}
        <i class="fa-solid fa-circle-xmark profile-search-result-platform-img-no-platform"></i>
        </a>
      </li>`;
    }
  }
